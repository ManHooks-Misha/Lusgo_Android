...
public class MonthwiseCalendarAdapter extends RecyclerView.Adapter<MonthwiseCalendarAdapter.MyViewHolder> {
    private ArrayList<HashMap<String, ArrayList<HashMap<String, List<Event>>>>> horizontalList;
    ...
    public MonthwiseCalendarAdapter(ArrayList<HashMap<String, ArrayList<HashMap<String, List<Event>>>>> horizontalList, Activity context, String usertype, String userid) {
...
//            ll_menu = view.findViewById(R.id.ll_menu);
    ...
    public void onBindViewHolder(@NotNull final MyViewHolder holder, final int position) {
        ...
        HashMap<String, ArrayList<HashMap<String, List<Event>>>> map = horizontalList.get(position);
       /* if(!key.isEmpty()) {
            key.
        }*/


        for (Map.Entry<String,  ArrayList<HashMap<String, List<Event>>>> pair : map.entrySet()) {
           holder.date.setText(pair.getKey());
           holder.date.setTextSize(20);
            ArrayList<HashMap<String, List<Event>>> arr_show = horizontalList.get(position).get(pair.getKey());

            MonthDateAdapter calenderAdapter = new MonthDateAdapter(
